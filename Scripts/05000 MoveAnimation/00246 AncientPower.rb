...
animation_user.play_on_bank(0, ya.create_sprite(:viewport, :sprite, Sprite, nil, [:load, 'AncientPower_0', :animation], [:set_rect, 0, 0, 104, 90], [:zoom=, 1.35], [:set_origin, -20, 90]))
animation_user.play_on_bank(1, ya.create_sprite(:viewport, :sprite, Sprite, nil, [:load, 'AncientPower_1', :animation], [:set_rect, 0, 0, 104, 90], [:zoom=, 1.35], [:set_origin, 120, 40]))
...
main_t_anim.play_before(ya.se_play('moves/AncientPower'))
...
Battle::MoveAnimation.register_specific_animation(:ancient_power, :first_use, animation_user, animation_target)
