...
animation_user.play_on_bank(0, ya.create_sprite(:viewport, :sprite, Sprite, nil, [:load, 'PainSplit', :animation], [:set_rect, 0, 0, 104, 90], [:zoom=, 2], [:set_origin, 10, 80]))
animation_user.play_on_bank(1, ya.create_sprite(:viewport, :sprite, Sprite, nil, [:load, 'PainSplit', :animation], [:set_rect, 0, 0, 104, 90], [:zoom=, 2], [:set_origin, 70, 40]))
...
main_t_anim.play_before(ya.se_play('moves/PainSplit'))
...
Battle::MoveAnimation.register_specific_animation(:pain_split, :first_use, animation_user, animation_target)
