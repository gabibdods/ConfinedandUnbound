...
animation_target.play_on_bank(1, ya.create_sprite(:viewport, :sprite, Sprite, nil, [:load, 'SunnyDay', :animation], [:set_rect, 0, 0, 540, 191], [:zoom=, 1], [:set_origin, 360, 120]))
animation_target.play_on_bank(0, ya.create_sprite(:viewport, :sprite, Sprite, nil, [:load, 'SunnyDay', :animation], [:set_rect, 0, 0, 540, 191], [:zoom=, 1], [:set_origin, 240, 190]))
...
main_t_anim.play_before(ya.se_play('moves/SunnyDay'))
...
Battle::MoveAnimation.register_specific_animation(:sunny_day, :first_use, animation_user, animation_target)
