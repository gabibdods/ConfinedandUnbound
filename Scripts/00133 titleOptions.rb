module UI
    ...
    class TitleControls < SpriteStack
      ...
      def initialize(viewport)
        super(viewport, 255, 240, default_cache: :title)
        # create_sprites
        # self.index = 0
      ...
      def create_play_bg
        @play_bg = add_sprite(213, 168, 'shader_bg')
        @play_bg.ox = @play_bg.width / 2
        @play_bg.shader = @shader
      end
  
      def create_credits_bg
        @credit_bg = add_sprite(213, 192, 'shader_bg')
      ...
      def create_play_text
        @font_id = 20
        add_text(213, 170, 0, 24, text_get(32, 77).capitalize, 1, 1, color: 9)
      end
  
      def create_credit_text
        @font_id = 20
        add_text(213, 194, 0, 24, 'Credits', 1, 1, color: 9)
      end
