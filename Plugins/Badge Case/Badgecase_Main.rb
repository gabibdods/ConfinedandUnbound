...
class Badgecase
  ...
  def get_all_regions
    ret = []
    all_badges_information.each do |badge|
      next if ret.include?(badge.region)
      ret.push(badge.region)
    end
    return ret
  end
end
...
class Player < Trainer
  attr_accessor :last_badge_count
  attr_accessor :last_badge_max

  alias org_init initialize
  def initialize(name, trainer_type)
    org_init(name, trainer_type)
    @last_badge_count = 0
    @last_badge_max = 0
  end

  def badge_count
    return @last_badge_count if !$PokemonGlobal
    @last_badge_count = $PokemonGlobal.badges.badge_count
    return @last_badge_count
  end

  def badge_max
    return @last_badge_max if !$PokemonGlobal
    @last_badge_max = $PokemonGlobal.badges.badge_max
    return @last_badge_max
  end
end
