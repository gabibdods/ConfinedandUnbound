...
class SceneMegaEvolution
  ...
  def start(time,backdrop,pkmn)
    ...
    17.times { @sprites["bg"].color.alpha += 15; pbWait(0.025) }
    pbWait(0.125)
    ...
    @sprites["pkmn"].color.alpha += 15; pbWait(0.025) }
    pbWait(0.05)
    # Set circle
    ...
    # Animation
    zoom = 0; 15.times { zoom += 0.2; set_zoom("circle",zoom,zoom); pbWait(0.05) }
    # Save color (pokemon)
    ...
    set_visible_sprite("circle"); pbWait(0.025)
    # Light
  ...
  def megaEvolve
    # Reset Form
    @sprites["pkmn"].bitmap = Bitmap.new(self.bitmapPkmn)
    17.times { @sprites["scene"].opacity -= 15; pbWait(0.05) }
    zoom = 2
    10.times { zoom-=0.1; set_zoom("pkmn",zoom,zoom); pbWait(0.05) }
    ...
    create_sprite("icon","icon_mega",@viewport,"UI/Battle")
...
#-------------------------------------------------------------------------------
  # Image
  def create_sprite(spritename,filename,vp,dir="/Plugins/Mega Evo Scene")
